...
after (states, labelsI, labelsO, transitions, init) sigma
    ...
    where afterImplementation = findAfter [init] sigma transitions
...
findFinalStateFromLabel l1 ((s1, label, s2) : transitions)
    | l1 == label || (label == "tau")
    = s2 : findFinalStateFromLabel l1 transitions
    |
...
prop_statesExist (states, labelsI, labelsO, transitions, init) labels =
    goThrough resultAfter states
  where
