...
--  LIMITATION: In LOC "(headOfPerms:_) = permutations [start..end]", we are selecting 
--              the FIRST permutation generated from set by Haskell's permutations function.
--  IMPROVEMENTS: It would be prefereable that a random permutation is selected, but 
--                we couldn't get that to work because of typing.
...
-- This is not a valid assumption so sometimes the tests will fail because 
-- the random arrays are permutations of each other
-- We planned to circumvent this by finding a way to tell genTwoIntList to filter, 
-- but that would need to call the isPermutation function... which is what we are testing
-- What this function does:
--      Runs a function 'fCounter' times and checks that the expected output is always correct
-- Improvements to the function:
--      Instead of expectedOut it should be a method of evaluation.
testRandomGenList :: Int -> Int -> ([Int] -> [Int] -> Bool)
                    -> Bool-> IO ()
testRandomGenList counter fCounter func expectedOut
 = if counter == fCounter then print (show fCounter ++ " tests passed")
                else do
                  (l1, l2) <- genTwoIntList
...
arrOfProps = [("Property: Two equivalent arrays", prop_baseCaseTrue),
              ("Property: Two different arrays", prop_baseCaseDifferent),
              ("Property: Valid Permutation of same set", prop_validPerm),
              ("Property: Different array lengths", prop_testDiffGeneratedLists)]
...
-- Rationale of Property Rankings (IMPORTANT !!!):
-- All properties have a ranking of 3. This means that all properties seem to have equivalent strength.
-- This is in line with what the sortability property of the isPermutation function implies.
-- None of the properties are a subset of each other!
-- However, let's say we tested that the first permutation in sorted manner is EQUIVALENT to the 
--   second permutation in sorted manner, IN COMPARISON to checking for length equivalence for first
--   and second permutation. The former is a stronger property then the latter, because not only we
--   are verifying same size of arrays, but also same content.
